...
function NavProfileCard({ profile, expanded }) {
    ...
    const resolvedProfilePictureUrl = profilePictureUrl ? utils.file.resolvePath(profilePictureUrl) : null

    return (
        <Card className={`nav-profile-card ${expandedClass}`}>
            <div className={`nav-profile-card-avatar-glitch`}>
                <ImageView src={profilePictureUrl}
                           className={`nav-profile-card-avatar`}
                           hideSpinner={true}
                           alt={name}/>
                {resolvedProfilePictureUrl && (<>
                    <div className="glitch-clone glitch-clone-1"
                         style={{backgroundImage: `url(${resolvedProfilePictureUrl})`}}/>
                    <div className="glitch-clone glitch-clone-2"
                         style={{backgroundImage: `url(${resolvedProfilePictureUrl})`}}/>
                </>)}
            </div>
